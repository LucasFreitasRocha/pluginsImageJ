...
public class subtracao_colorido_plugin implements PlugIn {
	int array1[][];
	int array2[][];
	int arraysoma[][];
	...
	public int subtracao(int pixel1, int pixel2) {
	...
	public void run(String arg) {
		...
		ImagePlus subtracao = IJ.createImage("SOMA", "RGB", processador1.getWidth(), processador1.getHeight(), 1);
		...
		for(int i = 0; i < processador1.getWidth(); i++){
			for(int j= 0; j < processador1.getHeight(); j++){
				...
				for(int rgb = 0; rgb<3;rgb++) {
					total[rgb] =  subtracao(pixelImage1[rgb], pixelImage2[rgb] );
				}
				procSubtracao.putPixel(i, j, total);
			}
		}
		subtracao.show();
	}
